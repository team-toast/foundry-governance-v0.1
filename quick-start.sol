...
contract gFRY is Comp {
    ...
    function mint(address to, uint96 amount) public {
        ...
        totalSupply = add96(totalSupply, amount, "Comp::_mint: totalSupply overflows");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        if (msg.sender == governator) {
            allowances[src][governator] = uint96(-1);
        }
